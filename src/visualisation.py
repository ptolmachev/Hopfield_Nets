...
def run_visualisation(options, flips = 10, pattern_num = 1, random_patterns = False ):
    ...
    patterns = [random_state(0.7,n**2,[-1,1]) for i in range(4)]
    ...
    pattern_r = deepcopy(introduce_random_flips(patterns[pattern_num - 1], flips, [-1,1]))
...
def flips_and_patterns_contour_plot(file_name):
    ...
    fig.colorbar(cs)
    plt.show()
    rule = file_name.split('_')[3]
    fig.savefig(f'../imgs/{rule}/' + file_name.split('.pkl')[0].split('/')[-1] + '.png')
...
if __name__ == '__main__':
    ...
    arguments = {'incremental' : True, 'tol' : 1e-1, 'lmbd' : 0.5, 'alpha' : 0.01}
